...
export class ReviewsService {
  ...
  constructor() { }
   ...
   private getAVGreviews(reviews:Review[],type?:Reliability):number{
    type ?  reviews = this.getReviewsOfType(reviews,type) :  reviews = this.getReviewsOfType(reviews)
    ...
    private getReviewsTypeCounts(reviews:Review[],type?:Reliability):number{
      if(type){
        return Number(reviews.filter(review=> review.reliability === type).length);
      }
